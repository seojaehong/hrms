...
## G. 07-07 F5/F3 기능 추가 (ralph 루프)
백로그 E절의 **F5(급여 엑셀 업/다운로드 + AI 스킬 연계)**·**F3(요청 보드)**를 framework-free 코어 + frappe 어댑터 패턴(insurance_filing_api.py 컨벤션)으로 신규 구현했다. F5는 급여대장 엑셀 다운로드(`download_payroll_workbook` — 해당 월 슬립을 노호 급여대장 포맷 xlsx + 합계행 1원 일치로 생성해 private File 반환), 업로드 검증(`validate_payroll_upload` — 파싱→무결성→기존 슬립과 diff 미리보기, 저장 없음), 업로드 확정(`apply_payroll_upload` — `human_approved` fail-closed 게이트 통과 시에만 슬립 upsert + Comment 감사)의 3단 API와 이를 묶은 급여 엑셀 업/다운로드 센터 PWA(`KoreaPayrollExcel.vue`, lime 히어로, isAdmin만 반영 버튼)로 구성되며, AI챗은 사이트 급여 데이터 질의를 감지해 read-only 집계 한 줄 요약(`2026-05 급여: 32명 · 실지급 합계 …원 · 마감 확정 대기`)을 답변 앞에 덧붙인다. F3는 `Korea Service Request` doctype(제목·분류·상태 전이 검증) + 생성/조회/상태변경 API 3종(생성 시 HR Manager Notification Log) + 요청 보드 PWA(`KoreaServiceRequests.vue`, cream 히어로, 상태 배지, HR Manager만 상태변경)로 구성된다. 엑셀 파싱/시더 로직은 `scripts/ingest/*`에서 `hrms/regional/south_korea/payroll_excel.py`로 승격하고 기존 스크립트는 thin wrapper로 하위호환을 유지했다. **검증**: 신규 테스트 5파일 53케이스(백엔드) + AI챗 +10 + 프론트 런타임 27케이스 GREEN, `scripts/run_korea_regional_smoke.py` → `passed: true`, `failed_count: 0`(96건).
