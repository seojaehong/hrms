...
- Gate 14 credential apply checkpoint result:
  ...
  - 2026-05-16 cron test evidence on `7d2072d92`: `python3 hrms/tests/test_korea_browser_credential_apply_checkpoint.py` passed 9 tests, including invalid required-scope fail-closed coverage.
  - 2026-05-16 cron runtime evidence on `7d2072d92`: `python3 scripts/verify_korea_demo_browser_credential_apply.py` failed closed before mutation because `FRAPPE_BROWSER_PASSWORD` was absent (`credential_apply.attempted: false`, `browser_verification.attempted: false`, `runtime_verified: false`, `fixture_fallback_required: true`).
  - 2026-05-16 prior runtime evidence on `abff52b09`: the first non-browser read-only verifier run found the Docker Bench app checkout stale versus the mounted workspace; restarting only the `frappe` service synced HRMS to `abff52b09`. The retry with `--include-bench --site hrms.localhost --company '노란봉투법 데모'` returned `runtime_verified: true`, source/app checkout matched the mounted workspace, positive scoped runtime rows were verified, and `fixture_fallback_required_until_positive_runtime_rows: false`. This is runtime source/row evidence, not authenticated browser proof.
