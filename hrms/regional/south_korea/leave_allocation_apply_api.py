"""Frappe-facing runtime apply API for Korea Leave Allocation.

Wraps ``apply_korea_leave_allocation`` with ``frappe.whitelist()`` so it is
callable from Frappe's REST/RPC layer.  All mutation decisions remain in the
core module; this file is a thin adapter only.
"""
...
@_whitelist
def apply_korea_leave_allocation_api(
    *,
    draft: Any,
    human_approved: Any,
    apply_actor: Any = None,
) -> dict[str, Any]:
    """Frappe whitelisted wrapper around apply_korea_leave_allocation.

    Parameters
    ----------
    draft:
        The ``draft`` payload from ``preview_korea_leave_allocation_draft``
        (dict or JSON string).
    human_approved:
        Must be the boolean ``True`` (or ``"true"``/``1`` from HTTP) to
        allow mutation.  Any falsy value triggers a fail-closed no-op.
    apply_actor:
    ...
    core = _load_sibling_module("leave_allocation_apply.py", "korea_leave_allocation_apply")
    return core.apply_korea_leave_allocation(
        draft=draft_payload,
...
__all__ = ["apply_korea_leave_allocation_api"]
