...
Status: active source-of-truth correction after Gate 14 credential-apply checkpoint, PR #210 kwargs/title fixes, and follow-up browser session/bootstrap hardening landed on `develop`; credential apply still requires an operator-provided secret plus explicit human approval, and positive authenticated browser proof is still unproven in cron until that approved run validates the current session-handoff fix.
...
- Latest Gate 14 checkpoint evidence: 2026-05-17 cron verification on `73680c445` passed the focused browser runtime test and credential-checkpoint tests; the non-browser Docker/Bench runtime verifier returned source-aligned positive scoped rows after restarting only the `frappe` service.
...
- 2026-05-17 non-browser runtime evidence on `73680c445` confirms the local Docker/Bench runtime is source-aligned and positive for scoped runtime rows after restarting only the `frappe` service (`runtime_verified: true`, `source_matches_mounted_workspace: true`, `positive_runtime_rows_verified: true`, `fixture_fallback_required_until_positive_runtime_rows: false`). This is not authenticated browser proof.
