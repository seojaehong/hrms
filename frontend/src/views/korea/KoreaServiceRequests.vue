			...
			<div class="flex flex-col my-7 p-4 gap-5">
				...
				<div class="flex flex-col gap-3">
					...
					>
						...
						<div class="text-xs text-[var(--k-ink-faint)]">{{ __('아직 접수된 요청이 없습니다.') }}</div>
					</div>
					...
					>
						<div class="flex justify-between items-center gap-2">
							<span class="text-sm font-semibold text-[var(--k-ink)]">{{ req.title }}</span>
							<span
						...
						<div v-if="req.detail" class="text-xs text-[var(--k-ink-muted)] whitespace-pre-wrap">{{ req.detail }}</div>
						...
						<div v-if="isAdmin && req.transitions.length" class="flex items-center gap-2 pt-1">
							<select
								...
								class="flex-1 border border-[var(--k-hairline)] rounded-lg px-2 py-1.5 text-xs text-[var(--k-ink)] bg-[var(--k-card)] focus:outline-none focus:ring-2 focus:ring-black/60"
							>
