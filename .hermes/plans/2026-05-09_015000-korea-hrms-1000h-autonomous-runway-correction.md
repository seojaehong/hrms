...
- Latest Gate 14 checkpoint evidence: 2026-05-16 cron verification on `b389fa336` confirmed `FRAPPE_BROWSER_PASSWORD` is absent, so the credential/browser checkpoint failed closed before mutation, skipped browser verification, and wrote a redacted JSON report artifact.
- Latest Gate 14 test evidence: focused checkpoint tests passed 10 tests on `b389fa336`, including report-file parent-directory/UTF-8 artifact coverage.
- Latest non-browser runtime-row evidence: the 2026-05-16 read-only runtime verification for company `노란봉투법 데모` on `b389fa336` returned `runtime_verified: true`, source/app checkout matched the mounted workspace, and positive scoped runtime rows were verified. This is runtime source/row evidence, not authenticated browser proof.
...
- `scripts/verify_korea_demo_browser_credential_apply.py` verifies the password env var and `--human-approved` flag before invoking `ensure_demo_browser_credential(..., human_approved=True)` through Docker Bench, rejects blank/whitespace-only passwords before apply, rejects blank required scope inputs before credential apply, tolerates Docker/Bench setup noise before the final JSON payload, creates parent directories for `--report-file`, writes UTF-8 JSON reports matching stdout, then runs `scripts/verify_korea_payroll_closing_browser_runtime.mjs` only if credential apply succeeds.
...
- 2026-05-16 non-browser runtime evidence on `b389fa336` confirms the local Docker/Bench runtime remains source-aligned and positive for scoped runtime rows (`runtime_verified: true`, `source_matches_mounted_workspace: true`, `positive_runtime_rows_verified: true`, `fixture_fallback_required_until_positive_runtime_rows: false`). This is not authenticated browser proof.
