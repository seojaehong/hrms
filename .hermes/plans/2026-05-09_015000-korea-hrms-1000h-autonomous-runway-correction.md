...
- Latest Gate 14 runtime evidence: 2026-05-16 cron verification on `abff52b09` confirmed `FRAPPE_BROWSER_PASSWORD` is absent, so the credential/browser checkpoint failed closed before mutation and skipped browser verification. The same run's first non-browser read-only runtime verification for company `노란봉투법 데모` found a stale Docker Bench app checkout versus the mounted workspace; restarting only the `frappe` service synced HRMS to `abff52b09`, and the retry returned `runtime_verified: true`, source/app checkout matched the mounted workspace, and positive scoped runtime rows were verified. This is runtime source/row evidence, not authenticated browser proof.
...
- `scripts/verify_korea_demo_browser_credential_apply.py` verifies the password env var and `--human-approved` flag before invoking `ensure_demo_browser_credential(..., human_approved=True)` through Docker Bench, rejects blank/whitespace-only passwords before apply, tolerates Docker/Bench setup noise before the final JSON payload, then runs `scripts/verify_korea_payroll_closing_browser_runtime.mjs` only if credential apply succeeds.
