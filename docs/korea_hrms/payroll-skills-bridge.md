...
| `~/.claude/skills/퇴직정산` | 퇴직소득세: 근속연수공제(소득세법 §48①1호, 5년/10년/20년 구간)→환산급여(공제후×12÷근속연수)→환산급여공제(§48①2호, 800만/7000만/1억/3억 구간)→과세표준×기본세율(§55①)=환산산출세액→÷12×근속연수=산출세액[10원 절사]→지방소득세(10%, 10원 절사) | `severance_settlement.py:calculate_severance_income_tax` | **일치 (2026-07-12 엔진 구현)** — 근속연수공제·환산급여공제·기본세율표를 소득세법 §48·§55 원문(법제처 API, mst 280405) 대조 확정. 저소득이라도 근속연수공제 초과분은 반드시 과세(0원 강제 금지, §48②는 미달분만 0 처리). |
| `~/.claude/skills/건보정산` | 건보정산: 월평균보수=보수총액÷산정월수, 확정보험료=ROUNDDOWN(월평균보수×3.595%,-1)×납부월수, 장기요양=ROUNDDOWN(확정건강보험료×13.14%,-1)×납부월수, 정산액=확정보험료−기납부(중도입사는 납부월수=산정월수−1) | `severance_settlement.py:reconcile_health_insurance_on_exit` | **일치 (2026-07-12 엔진 구현)** — 장기요양 환산율은 `statutory_2026.LONGTERM_CARE_RATE`(13.1405%) 단일소스 재사용, 스킬 표기 13.14%와 반올림 차이만(실질 동일). |
