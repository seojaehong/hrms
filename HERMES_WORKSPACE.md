...
- Gate 14 credential apply checkpoint result:
  ...
  - 2026-05-16 cron test evidence on `97344eb48`: `python3 hrms/tests/test_korea_browser_credential_apply_checkpoint.py` passed 11 tests, including browser-command-failure reporting coverage.
  - 2026-05-16 cron runtime evidence on `97344eb48`: `python3 scripts/verify_korea_demo_browser_credential_apply.py --report-file /tmp/korea-browser-credential-checkpoint-report-206.json` failed closed before mutation because `FRAPPE_BROWSER_PASSWORD` was absent (`credential_apply.attempted: false`, `browser_verification.attempted: false`, `runtime_verified: false`, `fixture_fallback_required: true`).
