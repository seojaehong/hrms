...
- Gate 14 credential apply checkpoint result:
  ...
  - 2026-05-16 cron runtime evidence on `9cf60a396`: `python3 scripts/verify_korea_demo_browser_credential_apply.py` failed closed before mutation because `FRAPPE_BROWSER_PASSWORD` was absent (`credential_apply.attempted: false`, `browser_verification.attempted: false`, `runtime_verified: false`, `fixture_fallback_required: true`).
  - 2026-05-16 cron runtime evidence on `9cf60a396`: the non-browser read-only runtime verifier with `--include-bench --site hrms.localhost --company '노란봉투법 데모'` returned `runtime_verified: true`, `source_matches_mounted_workspace: true`, `positive_runtime_rows_verified: true`, and `fixture_fallback_required_until_positive_runtime_rows: false`. This is runtime source/row evidence, not authenticated browser proof.
