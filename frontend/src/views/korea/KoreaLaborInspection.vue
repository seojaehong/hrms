		...
		<template #body>
			<div class="flex flex-col my-7 p-4 gap-5">
				...
				<div class="pt-1">
					...
					<div class="mt-1 text-xl font-bold tracking-tight text-black">{{ __('근로감독 대비 체크리스트') }}</div>
					<p class="mt-2 text-sm text-black/60">
						고용노동부 근로감독관 자율점검표 기반 15항목을 근거 조항·증빙서류·리스크와 함께 점검합니다.
					</p>
				</div>

				<div v-if="listInspectionChecklist.loading && !items.length" class="text-sm text-black/50 py-2">{{ __('불러오는 중...') }}</div>
				...
				<div v-if="items.length" class="k-block k-block--cream -mx-1">
					...
					<div class="mt-1 text-sm font-medium text-black/60">{{ __('점검 완료율') }}</div>
					...
					<div class="mt-1 text-xs text-black/50">{{ completedCount }} / {{ items.length }} {{ __('항목 완료') }}</div>
				</div>
				...
				<div v-for="group in groupedItems" :key="group.category" class="k-card p-0 overflow-hidden">
					...
					>
						<div class="flex items-center gap-2">
							...
							<span class="text-xs text-black/40">({{ group.items.length }})</span>
						</div>
						<div class="flex items-center gap-2">
							<span class="text-xs font-semibold px-2 py-0.5 rounded-full bg-black/10 text-black/60">
								{{ group.completedCount }}/{{ group.items.length }}
							</span>
							<span class="text-black/40 transition-transform" :class="{ 'rotate-180': openCategories[group.category] }">▾</span>
						</div>
					</button>
					...
					<div v-show="openCategories[group.category]" class="flex flex-col gap-2 px-4 pb-4">
						<div
							...
							:key="item.id"
							class="rounded-lg border border-black/10 p-3 flex flex-col gap-2"
						...
						>
							<label class="flex items-start gap-2 cursor-pointer">
								...
								<span class="text-sm font-medium text-black">
									<span class="text-black/40 mr-1">{{ item.id }}</span>{{ item.item }}
								</span>
							</label>

							<div class="pl-6 flex flex-col gap-1.5 text-xs text-black/60">
								<div><span class="font-semibold text-black/70">{{ __('근거') }}:</span> {{ item.legal_basis }}</div>
								<div>
									<span class="font-semibold text-black/70">{{ __('증빙서류') }}:</span>
									{{ item.evidence_needed.join(', ') }}
								</div>
								<div class="flex items-center gap-2 flex-wrap">
									<span class="font-semibold text-black/70">{{ __('리스크') }}:</span>
								...
								<div v-if="item.risk.note" class="text-black/40">{{ item.risk.note }}</div>
								<div class="flex items-center gap-2">
									<span class="font-semibold text-black/70">{{ __('자동점검') }}:</span>
									<span
										class="px-2 py-0.5 rounded-full font-semibold"
										:class="isAutomated(item) ? 'bg-blue-100 text-blue-700' : 'bg-black/10 text-black/50'"
									>
				...
				<div class="k-card p-3 text-xs text-black/60 leading-relaxed">
					<span class="font-semibold text-black">자율점검용 체크리스트입니다.</span>
