		...
		<template #body>
			<div class="flex flex-col my-7 p-4 gap-5">
				...
				<div class="k-block k-block--cream flex flex-col gap-2">
					...
					<div class="flex items-center gap-3">
						...
						<div class="flex flex-col gap-0.5">
							...
							<div class="text-xs text-black/50">{{ categoryDetail?.law ?? '' }}</div>
						</div>
						<div class="ml-auto">
							<span
								v-if="categoryDetail"
								class="text-xs font-medium px-2 py-0.5 rounded-full bg-white"
					...
					<div class="text-xs text-black/60 pt-1">
				...
				<div
					...
					class="k-card p-4 text-sm text-black/70"
				>
					카테고리 진단 데이터를 찾을 수 없습니다.<br />
					<router-link
						:to="{ name: 'KoreaComplianceDashboard' }"
						class="text-black font-semibold underline text-xs mt-1"
					>
						← 메인 대시보드로 돌아가기
					</router-link>
				</div>
				...
				<div v-if="loading" class="flex flex-col items-center py-10">
					<div class="text-black/40 text-sm">로딩 중...</div>
				</div>

				<template v-if="categoryDetail">
					<!-- 발견 사항 -->
					<div class="k-card p-4 flex flex-col gap-3">
						<div class="text-sm font-bold tracking-tight text-black">발견 사항</div>
						...
						>
							<div class="flex items-start justify-between gap-2">
								<div class="text-xs font-semibold text-black">
									...
									<span v-else class="text-black/40">직원 미특정</span>
								</div>
								<span class="text-xs text-black/40 shrink-0">
									{{ finding.law ?? '' }}
								</span>
							</div>
							<div class="text-xs text-black/70">{{ finding.issue }}</div>

							<!-- 상세 데이터 (wage delay: delay_days 등) -->
							<div v-if="finding.detail" class="mt-1 text-xs text-black/50 flex flex-wrap gap-3">
						...
						>
							<div class="text-xs text-black/60">{{ finding.issue }}</div>
							<div class="text-xs text-black/40">데이터 미등록 — 직접 확인 필요</div>
						</div>
						...
						<div class="flex items-center gap-2 pt-1">
							<label class="flex items-center gap-1.5 cursor-pointer">
								<input
									...
									class="rounded"
								/>
								<span class="text-xs text-black/60">직원 이름 마스킹</span>
							</label>
						</div>
					</div>
					...
					<div v-if="categoryDetail.recommendations?.length" class="k-card p-4 flex flex-col gap-3">
						<div class="text-sm font-bold tracking-tight text-black">개선 권고 체크리스트</div>
						...
						>
							...
							<label
								:for="`rec-check-${idx}`"
								class="text-xs text-black cursor-pointer leading-relaxed"
								:class="{ 'line-through !text-black/40': checkedRecommendations[idx] }"
							>
								{{ rec }}
							</label>
						</div>

						<div class="text-xs text-black/40 pt-1">
					...
					<!-- Audit Log (이 카테고리 한정) -->
					<div class="k-card p-4 flex flex-col gap-3">
						<div class="text-sm font-bold tracking-tight text-black">이력</div>

						<div v-if="auditLogs.length === 0" class="text-xs text-black/40 py-2 text-center">
							이 카테고리에 대한 이력이 없습니다.
						</div>
						...
						>
							<div class="flex items-center justify-between">
								<span class="text-xs font-medium text-black">{{ log.event_label }}</span>
								<span class="text-xs text-black/40">{{ log.timestamp }}</span>
							</div>
							<div class="text-xs text-black/50">{{ log.actor }}</div>
						</div>
					</div>

					<!-- 관련 모듈 링크 -->
					<div v-if="categoryDetail.relatedModules?.length" class="k-card p-4 flex flex-col gap-3">
						<div class="text-sm font-bold tracking-tight text-black">관련 모듈</div>
						<div class="flex flex-col gap-2">
							<a
								...
								class="text-xs text-black font-medium hover:underline flex items-center gap-1"
							>
								{{ mod.label }} →
							</a>
						</div>
					...
					<div class="flex">
						<router-link
							:to="{ name: 'KoreaComplianceDashboard' }"
							class="text-xs text-black/60 hover:underline"
						>
							← 대시보드로 돌아가기
						</router-link>
					</div>
			...
			>
				<div class="bg-white rounded-2xl shadow-xl p-6 mx-6 flex flex-col gap-4 max-w-sm w-full">
					<div class="text-base font-bold tracking-tight text-black">발견 사항 해결 표시</div>
