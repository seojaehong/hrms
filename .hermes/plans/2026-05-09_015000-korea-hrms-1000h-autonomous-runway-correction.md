...
- Latest Gate 14 checkpoint evidence: 2026-05-16 cron verification on `7d2072d92` confirmed `FRAPPE_BROWSER_PASSWORD` is absent, so the credential/browser checkpoint failed closed before mutation and skipped browser verification.
- Latest Gate 14 test evidence: focused checkpoint tests passed 9 tests on `7d2072d92`, including blank required-scope rejection before credential apply.
- Latest non-browser runtime-row evidence: the prior read-only runtime verification for company `노란봉투법 데모` on `abff52b09` found a stale Docker Bench app checkout versus the mounted workspace; restarting only the `frappe` service synced HRMS to `abff52b09`, and the retry returned `runtime_verified: true`, source/app checkout matched the mounted workspace, and positive scoped runtime rows were verified. This is runtime source/row evidence, not authenticated browser proof.
...
- `scripts/verify_korea_demo_browser_credential_apply.py` verifies the password env var and `--human-approved` flag before invoking `ensure_demo_browser_credential(..., human_approved=True)` through Docker Bench, rejects blank/whitespace-only passwords before apply, rejects blank required scope inputs before credential apply, tolerates Docker/Bench setup noise before the final JSON payload, then runs `scripts/verify_korea_payroll_closing_browser_runtime.mjs` only if credential apply succeeds.
- `hrms/tests/test_korea_browser_credential_apply_checkpoint.py` covers missing-secret fail-closed behavior, explicit approval gating, redacted reports/commands, blank required-scope fail-closed behavior, successful mocked apply+browser verification, browser-skip behavior when credential apply fails, and rejection of browser-positive output that omits the required read-only/human-approval/`assistant_only` safety metadata.
