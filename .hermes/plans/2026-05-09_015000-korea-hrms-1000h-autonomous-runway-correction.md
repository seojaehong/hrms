...
Status: checkpoint infrastructure, PR #210 kwargs/title fixes, follow-up session-handoff/bootstrap-call/report-file hardening, and credential-checkpoint child browser report-file pass-through are done and on `develop`; live positive authenticated browser proof remains pending because the credential apply/browser path still requires an operator-provided `FRAPPE_BROWSER_PASSWORD` plus explicit `--human-approved` execution.
...
- `scripts/verify_korea_demo_browser_credential_apply.py` verifies the password env var and `--human-approved` flag before invoking `ensure_demo_browser_credential(..., human_approved=True)` through Docker Bench, rejects missing and blank/whitespace-only passwords before apply with distinct report reasons, reports whitespace-aware required-scope availability flags, rejects blank required scope inputs before credential apply, tolerates Docker/Bench setup noise before the final JSON payload, creates parent directories for `--report-file`, writes UTF-8 JSON reports matching stdout, reports explicit apply/browser command-failure reasons, sends Bench kwargs as a Python literal rather than JSON booleans, passes a redacted `--browser-report-file` through to the child browser verifier when requested, then runs `scripts/verify_korea_payroll_closing_browser_runtime.mjs` only if credential apply succeeds.
