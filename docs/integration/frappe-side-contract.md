...
| `docs/korea/04-year-end-settlement.md` | 연말정산 7단계 계산 | 직접 반영 없음 | **불일치/누락** — 연말정산 결과 import/export 계약 부재 |
| `docs/korea/05-severance.md` | 퇴직금·퇴직소득세 계산 | 직접 반영 없음 | **불일치/누락** — severance 전용 payload, slip linkage, calc reference 부재 |
...
| `docs/korea/10-frappe-customization.md` | 커스텀 필드, Salary Slip 확장, 훅/리포트 | §9 Custom Doctype 설계 전반 | **부분 정합** — 확장 방향은 맞지만 문서가 제안한 `kr_resident_id` 저장과 본 계약의 PII 금지 원칙이 충돌 |

| 어긋난/누락 항목 | 관련 docs/korea 문서 | 영향 범위 | 우선순위 |
|---|---|---|---|
| 연말정산 결과 import/export 계약 없음 | `04-year-end-settlement.md` | 외부 엔진 결과를 Frappe에 정산 단위로 적재할 수 없음 | 높음 |
| 퇴직금/퇴직소득세 계약 없음 | `05-severance.md` | 퇴직자 payroll 흐름과 slip/audit 연결이 비어 있음 | 높음 |
| `kr_resident_id` 저장 제안과 API PII 금지 원칙 충돌 | `10-frappe-customization.md` | Employee 확장 설계와 통합 계약 보안 원칙 불일치 | 높음 |
