...
def calculate_annual_leave_entitlement(
	...
	_validate_inputs(hire_date, as_of_date, basis, fiscal_year_start_month, fiscal_year_start_day)

	service_years = completed_years(hire_date, as_of_date)

	if basis == "Fiscal Year":
		period_start, period_end = fiscal_period_for(
			as_of_date, fiscal_year_start_month, fiscal_year_start_day
		)
		monthly_accrual = first_year_monthly_accrual(hire_date, as_of_date)
...
def fiscal_year_annual_entitlement(
	...
	if completed_years(hire_date, as_of_date) < 1:
		...
		if employment_start > period_end:
			return 0
		employed_days = (period_end - employment_start).days + 1
		period_days = (period_end - period_start).days + 1
		return round(15 * employed_days / period_days, 2)
	return anniversary_annual_entitlement(completed_years(hire_date, period_end))
...
def fiscal_period_for(
	...
	start = dt.date(as_of_date.year, fiscal_year_start_month, fiscal_year_start_day)
	if as_of_date < start:
		start = dt.date(as_of_date.year - 1, fiscal_year_start_month, fiscal_year_start_day)
