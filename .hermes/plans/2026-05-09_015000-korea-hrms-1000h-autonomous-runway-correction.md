...
- Latest Gate 14 checkpoint evidence: 2026-05-17 cron verification on `980235b9a` passed the focused browser runtime test and 12 credential-checkpoint tests; the non-browser Docker/Bench runtime verifier returned source-aligned positive scoped rows.
...
- `scripts/verify_korea_demo_browser_credential_apply.py` verifies the password env var and `--human-approved` flag before invoking `ensure_demo_browser_credential(..., human_approved=True)` through Docker Bench, rejects missing and blank/whitespace-only passwords before apply with distinct report reasons, reports whitespace-aware required-scope availability flags, rejects blank required scope inputs before credential apply, tolerates Docker/Bench setup noise before the final JSON payload, creates parent directories for `--report-file`, writes UTF-8 JSON reports matching stdout, reports explicit apply/browser command-failure reasons, sends Bench kwargs as a Python literal rather than JSON booleans, then runs `scripts/verify_korea_payroll_closing_browser_runtime.mjs` only if credential apply succeeds.
