...
- Latest Gate 14 checkpoint evidence: 2026-05-16 cron verification on `97344eb48` confirmed `FRAPPE_BROWSER_PASSWORD` is absent, so the credential/browser checkpoint failed closed before mutation and skipped browser verification while writing a redacted JSON report artifact.
- Latest Gate 14 test evidence: focused checkpoint tests passed 11 tests on `97344eb48`, including browser verifier command-failure reason coverage.
- Latest non-browser runtime-row evidence: the 2026-05-16 read-only runtime verification for company `노란봉투법 데모` on `97344eb48` initially detected a stale Docker Bench app checkout; after restarting only the `frappe` service, it returned `runtime_verified: true`, source/app checkout matched the mounted workspace, and positive scoped runtime rows were verified. This is runtime source/row evidence, not authenticated browser proof.
...
- `scripts/verify_korea_demo_browser_credential_apply.py` verifies the password env var and `--human-approved` flag before invoking `ensure_demo_browser_credential(..., human_approved=True)` through Docker Bench, rejects blank/whitespace-only passwords before apply, rejects blank required scope inputs before credential apply, tolerates Docker/Bench setup noise before the final JSON payload, creates parent directories for `--report-file`, writes UTF-8 JSON reports matching stdout, reports explicit apply/browser command-failure reasons, then runs `scripts/verify_korea_payroll_closing_browser_runtime.mjs` only if credential apply succeeds.
- `hrms/tests/test_korea_browser_credential_apply_checkpoint.py` covers missing-secret fail-closed behavior, explicit approval gating, redacted reports/commands, blank required-scope fail-closed behavior, report-file artifact writing, successful mocked apply+browser verification, browser-skip behavior when credential apply fails, browser-command-failure reporting, and rejection of browser-positive output that omits the required read-only/human-approval/`assistant_only` safety metadata.
...
- 2026-05-16 non-browser runtime evidence on `97344eb48` confirms the local Docker/Bench runtime is source-aligned and positive for scoped runtime rows after restarting only the `frappe` service (`runtime_verified: true`, `source_matches_mounted_workspace: true`, `positive_runtime_rows_verified: true`, `fixture_fallback_required_until_positive_runtime_rows: false`). This is not authenticated browser proof.
