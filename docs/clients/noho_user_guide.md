...
## 5. 4대보험 신고 (취득·상실·근로내용확인)

입·퇴사자와 일용직 근무일을 시스템 데이터에서 뽑아 **취득신고서·상실신고서·근로내용확인신고서(일용직)** xlsx를 자동 생성합니다. 전자신고용 서식 그대로 채워져 나오므로 그대로 검토·제출할 수 있습니다.

**사용 절차 (3단계):**
1. **AI로 대상 추출** — 해당 월의 입사자(취득)·퇴사자(상실)·일용직 근무일을 시스템 근태·직원 데이터에서 자동 집계합니다.
2. **담당자 확인** — 추출된 대상 명단(성명·취득/상실일·상실사유·근무일수)을 담당 노무사가 확인합니다. 주민등록번호가 시스템에 없는 직원은 `rrn_missing` 명단으로 표시되니 별도 확인합니다.
3. **신고서 생성** — 담당자 확인(`human_approved`)이 끝난 대상에 한해서만 신고서 파일이 만들어집니다. 확인 전에는 **어떤 파일도 생성되지 않습니다**(오작성·오제출 방지 fail-closed 원칙).

⚠ 신고서는 담당자 승인 없이는 절대 자동 제출되지 않으며, 생성된 파일과 주민등록번호는 사업장 전용 공간에만 저장됩니다(공개 저장소 커밋 금지).
