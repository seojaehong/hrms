...
- Gate 14 credential apply checkpoint result:
  ...
  - 2026-05-16 cron test evidence on `b389fa336`: `python3 hrms/tests/test_korea_browser_credential_apply_checkpoint.py` passed 10 tests, including report-file artifact coverage.
  - 2026-05-16 cron runtime evidence on `b389fa336`: `python3 scripts/verify_korea_demo_browser_credential_apply.py --report-file /tmp/korea-browser-credential-checkpoint-report.json` failed closed before mutation because `FRAPPE_BROWSER_PASSWORD` was absent (`credential_apply.attempted: false`, `browser_verification.attempted: false`, `runtime_verified: false`, `fixture_fallback_required: true`).
  - 2026-05-16 cron non-browser runtime evidence on `b389fa336`: `python3 scripts/verify_korea_payroll_closing_runtime.py --include-bench --site hrms.localhost --company '노란봉투법 데모' --report-file /tmp/korea-payroll-closing-runtime-report.json` returned `runtime_verified: true`, source/app checkout matched the mounted workspace, positive scoped runtime rows were verified, and `fixture_fallback_required_until_positive_runtime_rows: false`. This is runtime source/row evidence, not authenticated browser proof.
