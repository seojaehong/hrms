...
- Gate 14 credential apply checkpoint result:
  ...
  - 2026-05-16 cron test evidence on `54b041d47`: `python3 hrms/tests/test_korea_browser_credential_apply_checkpoint.py` passed 11 tests, including browser-command-failure reporting and scope-report hardening coverage.
  - 2026-05-16 cron runtime evidence on `54b041d47`: `python3 scripts/verify_korea_demo_browser_credential_apply.py --report-file /tmp/korea-browser-credential-checkpoint-current.json` failed closed before mutation because `FRAPPE_BROWSER_PASSWORD` was absent (`credential_apply.attempted: false`, `browser_verification.attempted: false`, `runtime_verified: false`, `fixture_fallback_required: true`).
  - 2026-05-16 cron non-browser runtime evidence on `97344eb48`: the first read-only runtime verifier found positive scoped rows but detected the Docker Bench app checkout was stale; after restarting only the `frappe` service, `python3 scripts/verify_korea_payroll_closing_runtime.py --include-bench --site hrms.localhost --company '노란봉투법 데모' --report-file /tmp/korea-payroll-closing-runtime-report-206-after-restart.json` returned `runtime_verified: true`, source/app checkout matched the mounted workspace, positive scoped runtime rows were verified, and `fixture_fallback_required_until_positive_runtime_rows: false`. This is runtime source/row evidence, not authenticated browser proof.
...
- Latest checkpoint evidence:
  ...
  - Current no-approval/no-secret runs continue to return `credential_apply.attempted: false`, `credential_apply.reason: FRAPPE_BROWSER_PASSWORD missing`, `browser_verification.attempted: false`, `runtime_verified: false`, and `fixture_fallback_required: true`.
  ...
  - 2026-05-16 non-browser runtime evidence on `97344eb48` confirms the local Docker/Bench runtime is source-aligned and positive for scoped runtime rows after restarting only the `frappe` service. This is not authenticated browser proof.
