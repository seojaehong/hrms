		...
		<template #body>
			<div class="flex flex-col my-7 p-4 gap-5">
				...
				<div class="pt-1">
					...
					<div class="mt-1 text-xl font-bold tracking-tight text-black">{{ __('퇴직정산 (통합)') }}</div>
				...
				<div class="k-card p-4 flex flex-col gap-4">
					<div class="text-base font-bold tracking-tight text-black">{{ __('정산 조건 입력') }}</div>
					...
					<div class="flex items-center gap-2">
						...
						<label for="health-toggle" class="text-sm font-medium text-black">{{ __('건강보험 퇴직 정산 포함') }}</label>
					</div>
					<template v-if="useHealth">
						...
						<div class="flex items-center gap-2">
							...
							<label for="midmonth-toggle" class="text-sm text-black/70">{{ __('중도입사 (1일 입사 아님 — 입사월 보험료 미납)') }}</label>
						</div>
					</template>
				...
				<!-- 결과 카드 -->
				<div class="k-card p-4 flex flex-col gap-4">
					<div class="text-base font-bold tracking-tight text-black">{{ __('정산 결과') }}</div>

					<div v-if="severanceSettlement.loading" class="text-center py-8 text-black/40 text-sm">
						{{ __('정산 중...') }}
					</div>
					...
					<template v-else-if="result">
						...
						<div class="k-block k-block--cream -mx-1">
							...
							<div class="mt-1 text-sm font-medium text-black/60">실지급 총액</div>
						...
						<div class="flex flex-col gap-2">
							<div class="flex justify-between items-center border-b border-[var(--k-hairline-soft)] pb-2">
								...
								<span class="text-sm text-black k-numeric">{{ result.severance_pay?.continuous_service_days != null ? result.severance_pay.continuous_service_days + "일" : "-" }}</span>
							</div>
							<div class="flex justify-between items-center border-b border-[var(--k-hairline-soft)] pb-2">
						...
						<!-- 퇴직금 계산식 -->
						<div v-if="result.severance_pay?.calculation_formula" class="bg-[var(--k-surface-soft)] rounded-lg p-3 text-xs text-black/60 leading-relaxed">
				...
				<div class="k-card p-3 text-xs text-black/60 leading-relaxed">
					<span class="font-semibold text-black">참고용 정산입니다.</span>
